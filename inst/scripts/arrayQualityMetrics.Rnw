...
is the most appropriate to import your raw data into Bioconductor. To
learn how to produce an \Rclass{AffyBatch} from your data, please read
the documentation of the \Rpackage{affy} package.  Here, we use the
data set \emph{MLL.A}, an object of class \Rclass{AffyBatch} which is
provided in the data package \Rpackage{ALLMLL} for demonstration.
\emph{MLL.A} was created from CEL files using the function
\Rfunction{ReadAffy} from the \Rpackage{affy} package.
