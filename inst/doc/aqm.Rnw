...
%--------------------------------------------------
This vignette addresses an advanced topic: how to customize the
...
\emph{Microarray quality assessment with arrayQualityMetrics}.
There, you can learn the basic usage of the
\Rpackage{arrayQualityMetrics} package: how to use the 
\Rfunction{arrayQualityMetrics} function to create an array quality report
for a set of microarrays. 

\emph{Sections and modules:} In the documentation of this package, we
...
figure, some explanatory text, and potentially a set of flags that
classifies a subset of the arrays in the dataset as outliers
