%\VignetteIndexEntry{Working with arrayQualityMetrics report sections}
%\VignetteDepends{}
...
\title{Working with arrayQualityMetrics report modules}
...
In the other vignette of this package, \emph{Quality assessment with
  arrayQualityMetrics}, you can learn how to use the
\Rfunction{arrayQualityMetrics} function to create a full report. In this
vignette, we show how to produce individual report modules and
how to add your own modules to customize the report.
%

\section{Functions to format the data for aqm functions usage}

There are two functions to prepare the data. They compute intermediate
results that are then processed by the \Rfunction{aqm.xxx}
functions. They are greedy for memory and slow, but they only need to
run once, and this modular design allows us more flexibility and
extensibility with the report generation.

\begin{itemize}
\item \Rfunction{prepdata}: generates a \Rclass{aqmobj.prepdata} object which can be the input of all the \Rfunction{aqm.xxx} functions.
\item \Rfunction{prepaffy}: generates a \Rclass{aqmobj.prepaffy} object which is the input for some of the Affymetrix specific functions.
\end{itemize}
...
<<DataPreparation>>=
dataprep = prepdata(expressionset = nCCl4, intgroup = c(), do.logtransform = FALSE)
@ 
...
<<shortReport>>=
outdir = tempdir()
aqm.writereport(modules = qm, reporttitle = "My example", outdir = ".", arrayTable = data.frame())
outdir
@ 
