...
arrays within a data set. Some of the metrics can also be used to
diagnose batch effects, and thus the quality of the overall data set.
...
\subsection{Affymetrix data - before preprocessing}w
...
%--------------------------------------------------
We call the RMA algorithm on \emph{MLL.A} to obtain a preprocessed
...
Here, we consider the data set \Robject{CCl4} from the data package
...
\emph{replicate number}. Specifying a factor does not change 
...
(nonsensical) array metadata factors \Robject{condition} and
...
array. In the example of the data set \emph{CCl4}, the coordinates of
