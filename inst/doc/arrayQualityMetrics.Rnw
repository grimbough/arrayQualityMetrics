...
The function \Rfunction{arrayQualityMetrics} can be used on \Rclass{AffyBatch} for Affymetrix data sets, \Rclass{ExpressionSet} in the case of non Affymetrix one colour experiments, \Rclass{NChannelSet} or \Rclass{RGList} for dual colour experiments and \Rclass{BeadLevelList} for Illumina bead arrays. The call of the \Rfunction{arrayQualityMetrics} function is the same whatever the input object is. \Rfunction{arrayQualityMetrics} produces a \emph{HTML} report as an output.
The goal of this vignette is to show how to obtain a quality assessment report by the use of \Rfunction{arrayQualityMetrics}. With the most basic object of any of the class mentioned above, a report can be created. We will also show in this vignette how to customise the objects so that a more complete report is produced.

\section{Usage}

The call of the function is not dependent on the class of the object (\Rclass{NChannelSet}, \Rclass{AffyBatch}, \Rclass{ExpressionSet}, etc...) but it depends on the specificities that you want in the report.
The function \Rfunction{arrayQualityMetrics} is called
...
adviced to define a number of experiments to represent on the density plots. The default is FALSE, meaning that all the density curves are reprensented on the same plot.
\item \emph{intgroup}: is the name of the column in the \Rclass{phenoData} that contains the information about a covariate of interest to be shown as a side bar on the heatmap. The default name to this column is "Covariate".
...
\section{Output}
A report named \texttt{QMreport.html} is produced in the subdirectory given as 'outdir'. It contains text illustrated by \texttt{.png} files. Each \texttt{.png} is linked to corresponding \texttt{.pdf} files in order to provide high quality images. In the case of \Rclass{AffyBatch} input, some Affymetrix specific plots are added to the standard report. However the way of calling the function remains the same. As explained in the section \ref{ext}, some other plots can be added to the standard report if some specific columns are present in the input object.
...
We can use the \emph{MLL.A} \Rclass{AffyBatch} provided in the data package \Rpackage{ALLMLL} as an example to create a quality report.
...
Here is the simplest way of calling the function. We give a name to the directory ('outdir') and we force the existing files of this directory ('force'). Finally, we set 'do.logtransform' to TRUE as the data are raw.


\subsection{Example with an ExpressionSet}
If you are working on one colour non Affymetrix chips, you can load your data in Bioconductor as an \Rclass{ExpressionSet} object. Please see the documentation of the \Rpackage{Biobase} package to do so.
As an example for an \Rclass{ExpressionSet}, we can use the \emph{MLL.A} data after normalisation. 
...
Indeed, by normalising an \Rclass{AffyBatch} using RMA (\Rfunction{rma} function from the \Rpackage{affy} package), the output is an \Rclass{ExpressionSet}, as the probesets are summarised. We can then call the \Rfunction{arrayQualityMetrics} function the standard way:
...
@
%
As the data are already log transformed, we do not set 'do.logtransform' as the data are already log transformed (the default value being FALSE for this argument).


\subsection{Example with an NChannelSet}
...
As an instance of a \Rclass{NChannelSet}, we can use the \Rpackage{CCl4} data package. 

%
<<NChannelSet>>=
library("CCl4")
data(CCl4)
arrayQualityMetrics(expressionset = CCl4,
                    outdir = "CCl4",
                    force = TRUE,
                    do.logtransform = TRUE)
@
...
If you have Illumina bead arrays, you can build a \Rclass{BeadLevelList}, which is explained in the \Rpackage{beadarray}, and run \Rfunction{arrayQualityMetrics}.
It is also possible to use \Rfunction{arrayQualityMetrics} on \Rclass{RGList}, \Rclass{MAList}, \Rclass{marrayRaw} and \Rclass{marrayNorm}. More information about the \Rclass{RGList} and \Rclass{MAList} is given in the package \Rpackage{limma}. The objects \Rclass{marrayRaw} and  \Rclass{marrayNorm} are explained in the vignette of the package \Rpackage{marray}.

\section{Playing with the arguments}

\subsection{Intgroup and grouprep}

Some useful feature of \Rfunction{arrayQualityMetrics} is the possibility to show the results by taking into account a factor of interest, thanks to 'intgroup' and 'grouprep'. It does not change the way of computing the quality metrics. It just changes the colours of the boxplots and density plots and add a colour side bar to the heatmap.
...
@
%
With 'grouprep' setting to FALSE, we would only have the colour side bar on the heatmap. With 'grouprep' being TRUE, we have this colour side bar on the heatmap and the boxplots and density plots that are coloured according to the factor of interest (here, a,b,c,d).
\\
...
\subsection{Split.plots}
If we have a large number of arrays, the density plots are not readable. We thus can set the argument 'spli.plots' to a number of density curves to be shown on one plot.
...
We can use the \emph{CCl4} example again.

\subsection{Data import}
We first need to load the data. As we have already done a report on the non normalised data, we can normalise the data using the variance stabilization method available in the package \Rpackage{vsn}.
...
\subsection{X and Y coordinates of the spots}
...
A report named \texttt{QMreport.html} is produced in the subdirectory \emph{CCl4}. As 'grouprep' is TRUE and 'intgroup' is "RNAintegrity", the boxplot and density plots are represented with colours depending on the "RNAintegrity" value and a side bar is drawn next to the heatmap according to this factor as well.
