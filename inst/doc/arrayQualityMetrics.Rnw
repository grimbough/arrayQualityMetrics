...
%\VignetteDepends{arrayQualityMetrics,CCl4, ALLMLL, vsn}
...
report is produced in the subdirectory specified by the function's
\texttt{outdir} argument (the name of the central HTML page that can
be opened by a browser is \texttt{index.html}).  The report contains a
series of plots explained by text.  The plots are provided as bitmaps
(PNG format) or as vector graphics (SVG format). In the former case,
the figures are also linked to PDF files that provide high resolution
versions e.\,g.\ for publication.
...
metadata are present in the input object (see Section~\ref{ext}).
...
\Rclass{ExpressionSet} object \footnote{See the documentation of the
  \Rpackage{Biobase} package.} and proceed exactly as above.

\subsection{Two colour arrays}
...
\Rclass{NChannelSet} from scratch. Furthermore, the package
\Rpackage{limma} can import a wide range of data formats used for two
colour arrays, and will produce objects of class \Rclass{RGList},
\Rclass{MAList}. \Rfunction{arrayQualityMetrics} also understands these.

Here, we consider the dataset \Robject{CCl4} from the data package
...
\subsection{Illumina beadarrays}

For Illumina bead arrays, you can build a
\Rclass{BeadLevelList}, which is explained in the \Rpackage{beadarray}
package, and again run the function \Rfunction{arrayQualityMetrics}.  
...
how the quality metrics are computed. However, by setting the argument
...
arrayQualityMetrics(expressionset = nMLL,
                    outdir = "Report for nMLL with factors",
                    force = TRUE,
...
and "Y" respectively, so that these coordinates are taken into account
and the spatial distribution of the intensities can be drawn.
...
<<NChannelSet, results=hide>>=
arrayQualityMetrics(expressionset = nCCl4,
                    outdir = "Report for nCCl4 with RIN",
                    force = TRUE,
