...
\renewcommand{\thetitle}{Introduction}
...
%
<<DataLoading,results="hide",error=FALSE>>=
library("ALLMLL")
data("MLL.A")
@
...
%
<<AffyBatchQM,results="hide",error=FALSE>>=
...
for each array.
%
<<Normalisation, results="hide",error=FALSE>>=
nMLL = rma(MLL.A)
@
%
We can then call again the function \Rfunction{arrayQualityMetrics}.
%
<<ExpressionSet,results="hide",error=FALSE>>=
...
%
<<NChannelSet1,results="hide",error=FALSE>>=
library("vsn")
library("CCl4")
data("CCl4")
...
and "Y" respectively
%
<<XYcoordinates,results="hide",error=FALSE>>=
...
%
<<NChannelSet2, results="hide",error=FALSE>>=
arrayQualityMetrics(expressionset = nCCl4,
...
<<pkgs, results="tex", echo=FALSE,error=FALSE>>=
toLatex(sessionInfo())
@
%
