   ...
   if(all(intgroup %in% names(phenoData(expressionset)@data)))
      {
        ...
        colourCov = lapply(seq(along = intgroup), function(i) brewer.pal(8,rownames(brewer.pal.info[brewer.pal.info$category=="qual",])[i]))
    ...
    legend = sprintf("The <b>figure <!-- FIG --></b> shows a false color heatmap of between arrays distances, computed as the mean absolute difference (L<sub>1</sub>-distance) of the vector of M-values for each pair of arrays. <br><i>d<sub>xy</sub> =  mean|M<sub>xi</sub>-M<sub>yi</sub>|</i><br> Here, <i>M<sub>xi</sub></i> is the M-value of the <i>i</i>-th probe on the <i>x</i>-th array, %s Consider the following decomposition of <i>M<sub>xi</sub>: M<sub>xi</sub> = z<sub>i</sub> + &beta;<sub>xi</sub> + &epsilon;<sub>xi</sub></i>, where <i>z<sub>i</sub></i> is the probe effect for probe <i>i</i> (the same across all arrays), <i>&epsilon;<sub>xi</sub></i> are i.i.d. random variables with mean zero and <i>&beta;<sub>xi</sub></i> is such that for any array <i>x</i>, the majority of values <i>&beta;<sub>xi</sub></i> are negligibly small (i. e. close to zero). <i>&beta;<sub>xi</sub></i> represents differential expression effects. In this model, all values <i>d<sub>xy</sub></i> are (in expectation) the same, namely 2 times the standard deviation of <i>&epsilon;<sub>xi</sub></i>. The colour scale is chosen to cover the range of L<sub>1</sub>-distances encountered in the dataset. Arrays whose distance matrix entries are way different give cause for suspicion, so this plot can serve to detect outlier arrays. The dendrogram on this plot also can serve to check if, without any probe filtering, the arrays cluster accordingly to a biological meaning.",  leghmspe)
