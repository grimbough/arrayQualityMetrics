...
setMethod("arrayQualityMetrics",signature(expressionset = "NChannelSet"),
          function(expressionset, outfile, log.transformed, numberofgraphs)
          {
            ...
            titletext = sprintf("<hr><h1><center>%s</h1></center><hr><table border = \"0\" cellspacing = 5 cellpadding = 2>", title)
...
##########################################
            ...
            section = 1
            figure = 1
            sec1text = sprintf("<h2>Section %s: Individual array quality</h2>", section)
...
#################################
            ...
            if("r" %in% rownames(featureData(expressionset)@varMetadata) && "c" %in% rownames(featureData(expressionset)@varMetadata))
              {
                section = section + 1
                sec2text = sprintf("<h2>Section %s: Spatial Effects</h2>", section)
...
#################################
            ...
            if("replicates" %in% names(phenoData(expressionset)))
              {
                section = section + 1
                sec3text = sprintf("<h2>Section %s: Reproducibility</h2>", section)          
                ...
                legendrepli = sprintf("<DIV STYLE=\"text-align:justify;\"><b>Figure %s</b> represents the comparison of the log2 ratio of the replicate arrays. Each pairs of replicates are directly compared thanks to a scatter plot representation and a correlation coefficient is calculated. Scatterplot with a wide distribution of the dots and with a low correlation coefficient means a bad reproducibility of the experiment.</DIV><hr>", figure)          
...
#################################################
            ...
            ##Boxplots
            section = section + 1
            sec4text = sprintf("<h2>Section %s: Homogeneity between experiments</h2>", section)
            ...
            if(max(group) > 1)
              {
                ...
                dev.off()
                dtext = sprintf("<tr><td><b>%s</b></td><td><A HREF=\"%s\"><center><IMG BORDER = \"0\" SRC=\"%s\"/></A></center></tr></td></table><hr>\n", "Density plots", dpdf, dpng)
...
########################################
            ...
            if("GC" %in% rownames(featureData(expressionset)@varMetadata))
              {
                section = section + 1
                sec5text = sprintf("<h2>Section %s: Array platform quality</h2>", section)    
            ...
            if("gn" %in% rownames(featureData(expressionset)@varMetadata))
              {
                ...
                legendgo = sprintf("<DIV STYLE=\"text-align:justify;\"><b>Figure %s</b> shows the density distributions of the log2 ratios grouped by the mapping of the probes. Blue, density estimate of intensities of segments that overlap an annotated mRNA in RefSeq database. Gray, segments that do not overlap a Refseq mRNA.</DIV><hr>",  figure)          
...
##########################################
            ##Heatmap
            section = section + 1
            sec6text = sprintf("<h2>Section %s: Between array comparison</h2>", section)
...
aqm.expressionset = function(expressionset, outfile, log.transformed, numberofgraphs, con)
  {
    ...
    if(max(group) > 1)
      {
        ...
        dev.off()
        dtext = sprintf("<table cellspacing = 5 cellpadding = 2><tr><td><b>%s</b></td><td><A HREF=\"%s\"><IMG BORDER = \"0\" SRC=\"%s\"/></A><b>Figure %sb</b></CENTER><BR></tr></td></table><hr>\n", "Density plots", dpdf, dpng, figure)
        writeLines(dtext, con)
      }
    legendhom = sprintf("<DIV STYLE=\"text-align:justify;\">The quality metrics in this section look at the distribution of the (raw, unnormalized) feature intensities for each array. <b>Figure %sa</b> shows density estimates (histograms), and <b>Figure %sb</b> presents boxplots of the same data. Arrays whose distributions are very different from the others should be considered for possible problems.</DIV><hr>", figure, figure)          
    writeLines(legendhom, con)
...
##########################################
    ...
    axis(2, label= as.list(pretty(outM,9)),at=seq(0,1,by=(1/(length(pretty(outM,9))-1))), cex.axis = 0.8, padj = 1)
    dev.off()
    hltext = sprintf("<td><IMG BORDER = \"0\" SRC=\"%s\"/></CENTER><BR></td></tr></table><hr>\n",  hlpng)
    writeLines(hltext, con)
    legendheatmap = sprintf("<DIV STYLE=\"text-align:justify;\"><b>Figure %s</b> shows a false color heatmap of between arrays distances, computed as the MAD of the M-value for each pair of arrays. <br>dij = c.median|xmi-xmj|<br> Here, xmi is the normalized intensity value of the m-th probe on the i-th array, on the original data scale. c = 1.4826 is a constant factor that ensures consistency with the empirical variance for Normally distributed data (see manual page of the mad function in R). This plot can serve to detect outlier arrays. Consider the following decomposition of xmi: xmi = zm + Bmi + Emi, (1)where zm is the probe effect for probe m (the same across all arrays), Emi are i.i.d. random variables with mean zero and Bmi is such that for any array i, the majority of values Bmi are negligibly small (i. e. close to zero). Bmi represents differential expression effects. In this model, all values dij are (in expectation) the same, namely sqrt(2) times the standard deviation of Emi . Arrays whose distance matrix entries are way different give cause for suspicion. The dendrogram on this plot also can serve to check if, without any probe filtering, the experiments cluster accordingly to a biological meaning.</DIV><hr>",  figure)          
    writeLines(legendheatmap, con)

    l = list(numArrays,sN, section, figure)
    return(l) 
  }
...
############################
            ...
            section = section + 1
            sec7text = sprintf("<h2>Section %s: Affy plots</h2>", section)
            ...
            dev.off()
            dev.off()
            affytext = sprintf("<table cellspacing = 5 cellpadding = 2><tr><td><b>%s</b></td><td><A HREF=\"%s\"><IMG BORDER = \"0\" SRC=\"%s\"/></A><BR><b>Figure %s</b></CENTER></tr></td><tr><td><b>%s</b></td><td><A HREF=\"%s\"><IMG BORDER = \"0\" SRC=\"%s\"/></A><BR><b>Figure %s</b></CENTER></tr></td><tr><td><b>%s</b></td><td><A HREF=\"%s\"><IMG BORDER = \"0\" SRC=\"%s\"/></A><BR><b>Figure %s</b></CENTER></tr></td></table><hr>\n", "RNA degradation plot", affypdf1, affypng1, figure1, "RLE plot", affypdf2, affypng2, figure2, "NUSE plot", affypdf3, affypng3, figure3)
...
</DIV><hr>",  figure1, figure2, figure3)          
